...
import (
	...
	"github.com/sirupsen/logrus" // Изменено: Используем logrus напрямую
	"gorm.io/gorm"
)
...
type categoryService struct {
	...
	logger       *logrus.Logger // Изменено: Используем logrus напрямую
}

// NewCategoryService создает новый экземпляр CategoryService.
func NewCategoryService(categoryRepo repository.CategoryRepository, logger *logrus.Logger) CategoryService { // Изменено
...
func (s *categoryService) CreateCategory(userID uuid.UUID, name, categoryType string) (*model.Category, error) {
	...
	category := &model.Category{
		ID:     uuid.NewString(), // Исправлено: прямое присвоение uuid.UUID
		UserID: userID.String(),  // Исправлено: прямое присвоение uuid.UUID
		Name:   name,
		Type:   categoryType,
	}
